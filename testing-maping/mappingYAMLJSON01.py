...
def apply_feature_mapping(yaml_data, feature_map, auxFeaturesAddedList, aux_hierchical_prop):
    """
        ...
        for key, value in yaml_data.items():
                ...
                if isinstance(value_features, str) and value_features.endswith(key) and value_features not in auxFeaturesAddedList: ## key_features.endswith(key) # Salida similar
                    if key_features.count("_") == 2: # len(auxFeaturesAddedList) < 3 and
                        ...
                        if any(feature.endswith(key) for feature in auxFeaturesAddedList): ## and aux_feature_before_map not in auxFeaturesAddedList
                            ...
                            if key_features not in aux_hierchical_prop and aux_feature_before_insertion in auxFeaturesAddedList: ##aux_feature_before_insertion in auxFeaturesAddedList:
                                key = value_features
                                print(f"Valor finalmente insertado en la lista: {key_features}  {value_features}")
                                print(f"Lista que se inserta en el if: {auxFeaturesAddedList}")
                                auxFeaturesAddedList.add(value_features)
            ...
            if aux_nested:
                ...
                if aux_maps: ## quizas llevar esta condicion a otra parte, es "especial"
                    ...
                    new_data[mapped_key] = [apply_feature_mapping(item, feature_map, auxFeaturesAddedList.copy(), aux_hierchical_prop) if isinstance(item, (dict, list)) else item for item in value] ## auxFeaturesAddedList: antes de la mod
            else:
                #if isinstance (value, int):
                #    print(f"ERROR EN EL FILE ELSE FINAL: {mapped_key}   {value} {key}")
                new_data[mapped_key] = apply_feature_mapping(value, feature_map, auxFeaturesAddedList, aux_hierchical_prop) if isinstance(value, (dict, list)) else value
...
for filename, index, yaml_data, simple_props, hierarchical_props, key_value_pairs, root_info in yaml_data_list:
    ...
    updated_config = apply_feature_mapping(yaml_data, feature_map, auxFeaturesAddedList, aux_hierchical)
